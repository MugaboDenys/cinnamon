...
const Topicscard = ({image, title, author, date, time, topic}) => {
  ...
  return (
    <Link href={"/"} className="text-black hover:text-indigo-600">
      <Image alt="" src={image} width={300} />
      <h5 className="text-sm font-bold pt-3">{title}</h5>
      <h4 className="text-neutral-600 pt-3">{author} <span className={dot}>.</span> {date} <span className={dot}>.</span> {time}</h4>
      <h3 className="text-2xl font-bold pt-3">{topic}</h3>
    </Link>
  );
};
